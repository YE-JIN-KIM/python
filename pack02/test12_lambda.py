...
# 일급함수 : 함수안에 함수 선언 가능, 인자로 함수 전달, 반환값이 함수 인 경우
from pack01.test4_re import result
...
print((lambda x, y:x + y)(1, 2))  # 위 함수랑 같은 뜻 

jtbc = lambda x,y:x*y   # lambda 함수의 주소를 가짐 
print(jtbc)  # <function <lambda> at 0x1057dd1c0>
print(jtbc(3, 4))
...
# Q1) 1 ~ 100 사이의 정수 중 5의 배수이거나 7의 배수만 걸러 보기 
print(list(filter(lambda a:a % 5 == 0 or a % 7 == 0, range(1,101))))

print('\n함수 장식자 : @함수명 - - meta 기능을 갖음')
...
# 위에 78 ~ 79줄의 내용을 밑에 함수로 함축 ( 데코레이터 사용) 
@make2  # 감싸주는 기능 
@make1
def hello2():
    return '신기해'
...
print('계승(factorial : 1부터 어떤 양의 정수 n 까지의 수를 모두 곱한 것) 처리를 위한 함수')
def myFact(a):
    if a == 1: return 1
    print(a)
    return a * myFact(a - 1)
